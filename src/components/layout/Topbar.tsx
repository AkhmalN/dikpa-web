  ...
  return (
    <header className="h-[60px] flex items-center border-b border-border bg-background px-4 gap-3">
      ...
      <div className="flex items-center gap-2">
        {/* Notification bell */}
        <Button variant="ghost" size="icon-sm" className="relative size-8 rounded-full bg-[#F5F5F5] hover:bg-[#E5E5E5]">
          ...
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 rounded-[4px] px-2 py-1.5 hover:bg-[#F5F5F5] transition-colors outline-none">
              ...
              <div className="hidden sm:flex flex-col items-start">
                <span className="text-[12px] font-medium text-foreground leading-tight">{user?.full_name}</span>
                <span className="text-[11px] text-muted-foreground capitalize leading-tight">{user?.role?.replace("_", " ")}</span>
              </div>
            </button>
          </DropdownMenuTrigger>
